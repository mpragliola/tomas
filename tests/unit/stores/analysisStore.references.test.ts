 ...
 * `wavParser.test.ts` / `core-flow.test.ts` — no mocking of `parseAudioFile`.
 */
...
describe('analysisStore references', () => {
  ...
  describe('addReference', () => {
    it('adds up to MAX_REFERENCES and rejects the one past it', async () => {
      ...
      for (let i = 0; i < store.MAX_REFERENCES; i++) {
        const id = await store.addReference(toneFile('sine-1k'));
      ...
      const rejectedId = await store.addReference(toneFile('sine-1k'));
      expect(rejectedId).toBe('');
      expect(store.referenceOrder.length).toBe(store.MAX_REFERENCES);
      expect(store.lastError).not.toBeNull();
    });

    it('activates the first reference added, and only the first', async () => {
      const store = useAnalysisStore();
      await store.loadFile(toneFile('harmonic-e2'));

      const first = await store.addReference(toneFile('white-noise'));
      expect(store.activeReferenceId).toBe(first);

      const second = await store.addReference(toneFile('white-noise-eq'));
      expect(store.activeReferenceId).toBe(first);
      expect(store.activeReferenceId).not.toBe(second);
    });
  });
  ...
  describe('cloneReference', () => {
    it('shares assetId but gets a distinct id and independent computed state', async () => {
      const store = useAnalysisStore();
      await store.loadFile(toneFile('harmonic-e2')); // 1s @ 48kHz — exactly the analysis floor

      const sourceId = await store.addReference(toneFile('white-noise')); // 2s, flat noise
    ...
    it('rejects cloning past MAX_REFERENCES', async () => {
      const store = useAnalysisStore();
      await store.loadFile(toneFile('sine-1k'));
      const sourceId = await store.addReference(toneFile('sine-1k'));
      for (let i = 1; i < store.MAX_REFERENCES; i++) {
        await store.addReference(toneFile('sine-1k'));
      }
      expect(store.referenceOrder.length).toBe(store.MAX_REFERENCES);
  ...
  describe('removeReference', () => {
    it('garbage-collects an orphaned asset but keeps one a sibling clone still points at', async () => {
      ...
      const id1 = await store.addReference(toneFile('white-noise'));
    ...
    it('activates a sibling when the active tab is removed', async () => {
      ...
      const id1 = await store.addReference(toneFile('sine-1k'));
      const id2 = await store.addReference(toneFile('sine-1k'));
      const id3 = await store.addReference(toneFile('sine-1k'));
      expect(store.activeReferenceId).toBe(id1);

      store.removeReference(id1);
      await flush();
      // id2 was next in referenceOrder after id1.
  ...
  describe('setActiveReference', () => {
    it('recomputes a stale tab before activating it', async () => {
      const store = useAnalysisStore();
      await store.loadFile(toneFile('harmonic-e2'));
      const id = await store.addReference(toneFile('white-noise'));
    ...
    it('activating a second reference does not disturb the first one', async () => {
      ...
      const id2 = await store.addReference(toneFile('white-noise-eq'));
      expect(store.references[id2]!.stale).toBe(true); // not auto-activated

      store.setActiveReference(id2);
      await flush();

      expect(store.activeReferenceId).toBe(id2);
      expect(store.references[id2]!.stale).toBe(false);
